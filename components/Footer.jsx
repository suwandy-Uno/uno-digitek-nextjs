    ...
    <footer className="bg-navy border-t border-gold/20">
      <div className="max-w-7xl mx-auto px-8 py-16 grid grid-cols-1 md:grid-cols-4 gap-12">
        ...
        <div className="md:col-span-1">
          ...
          <p className="text-white/40 text-sm leading-relaxed font-body mb-6">{x.tagline}</p>
          <p className="text-white/30 text-xs font-body">{x.moto}</p>
        </div>

        {/* Solutions */}
        <div>
          <h4 className="text-gold text-sm font-bold tracking-widest uppercase font-body mb-5">
            {x.solutions}
          </h4>
          <ul className="flex flex-col gap-3">
            {x.solutionLinks.map((l) => (
              <li key={l.label}>
                <Link href={l.href} className="text-white/50 hover:text-gold text-sm font-body transition-colors">
                  {l.label}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="flex flex-col gap-3 text-white/50 text-sm font-body">
